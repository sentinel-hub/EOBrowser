...
import PREDEFINED_LAYERS_METADATA from '../components/Legend/layers_metadata.json';

/*
returns legend definition from layers_metadata.json for provided datasource and layer.
*/
export function findMatchingLayerMetadata(datasourceId, layerId) {
...
export async function getLegendImageURL(legendData) {
  ...
  let legendSpec = findMatchingLayerMetadata(legendData.datasourceId, legendData.layerId);
  let xmlData = undefined;
  if (legendSpec) {
    try {
      xmlData = createSVGLegend(legendSpec);
      return 'data:image/svg+xml;base64,' + b64EncodeURIComponentUnicode(xmlData);
    } catch (e) {
...
export const extractLegendDataFromPreset = (datasourceId, datasourceName, layerId) => {
  ...
  const hasLegend =
    preset &&
    (findMatchingLayerMetadata(datasourceId, layerId) || preset.legendUrl || preset.legendDefinitionJsonUrl);
...
function b64EncodeURIComponentUnicode(str) {
