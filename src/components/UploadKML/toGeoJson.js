// Source https://mapbox.github.io/togeojson/
// Functionality has been changed at coord() function. Will ignore the height element.
...
var toGeoJSON = (function() {
  ...
  function coord(v) {
    ...
    for (var i = 0; i < coords.length; i++) {
      o.push([coord1(coords[i])[0], coord1(coords[i])[1]]);
    }
    return o;
  }
  ...
  var t = {
    kml: function(doc) {
      ...
      if (latLonBox && latLonBox.length > 0) {
        gj.features = [getLatLon(latLonBox[0])];
      }
      ...
      function getLatLon(root) {
        var north = parseFloat(root.children[0].innerHTML),
          south = parseFloat(root.children[1].innerHTML),
          east = parseFloat(root.children[2].innerHTML),
          west = parseFloat(root.children[3].innerHTML);
        return {
          type: 'Feature',
          geometry: {
            type: 'Polygon',
