...
class AdvancedHolder extends React.Component {
  ...
  render() {
    const { channels, selectedResult: { layers, evalscript, evalscripturl, datasource}, currView, isEvalUrl, style, views } = this.props
        ...
        <div className="advancedPanel" style={style}>
          ...
          {isScript ? (
            ...
            <BandsPanel
              channels={dsChannels}
              layers={layers}
              onDrag={layers => Store.setLayers(layers)}
            />
          )}
        </div>
      ) : <div />
