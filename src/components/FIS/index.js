...
const tooltipContent = (dataPoint) => (
  <div>
    <div>{moment(dataPoint.date).format('ddd, DD. MMM YYYY')}</div>
    <ul>
      <li>mean: {dataPoint.mean.toFixed(2)}</li>
      <li>P<sub>10</sub> / P<sub>90</sub>: {dataPoint.p10.toFixed(2)} {dataPoint.p90.toFixed(2)}</li>
    </ul>
    <hr />
    <ul>
      <li>median: {dataPoint.median.toFixed(2)}</li>
      <li>st. dev.: {dataPoint.stDev.toFixed(2)}</li>
      <li>min / max: {dataPoint.min.toFixed(2)} {dataPoint.max.toFixed(2)}</li>
    </ul>
  </div>
);
  ...
  return (
    <CCSliderStyled>
      ...
      <RCSlider
        ...
        max={100}
        step={1}
        defaultValue={props.cloudCoverPercentage}
        onChange={(value) => { props.onChange && props.onChange(value) }}
      />
      <i className="fa fa-cloud">&nbsp;</i>
      <span className="percentage">{props.cloudCoverPercentage}&nbsp;%</span>
    </CCSliderStyled>
  )
}
...
export const getFisShadowLayer = (instanceName, originalLayerId) => {
  ...
  return fisShadowLayers[instanceName].find((l) => (l.id === `__FIS_${originalLayerId}`));
}
...
export default class FIS extends Component {
  ...
  constructor(props) {
    ...
    const layerInfo = presets[selectedResult.name].find((layer) => (layer.id === selectedResult.preset));
    ...
    this.state = {
      fetching: true,
      title: `${selectedResult.name} - ${layerInfo.name}`,
  ...
  requestFISData(fromDate, toDate) {
    if (this.requestInProgress === null) {  // this is the first time we are issuing this request
    ...
    // start request:
    request.get(this.requestInProgress.url, { params: this.requestInProgress.params })
      .then(res => {
        this.requestInProgress = null;
        this.onDataReceived(fromDate, toDate, res.data);
  ...
  constructRequestParameters(fromDate, toDate) {
    const { aoiBounds, presets, selectedResult } = Store.current;
    const geom = WKT.convert(new Terraform.Primitive(cloneDeep(aoiBounds.geometry)));
    const areaM2 = parseFloat(geo_area.geometry(aoiBounds.geometry));
    const resolution = Math.max(
      selectedResult.resolution || 10,  // resolution of the layer is a minimal allowed value
      Math.ceil(Math.sqrt(areaM2 / 500)),  // we would like to get a bit more than 500 points
    );
    ...
    if (shadowLayerInfo) {
      url = selectedResult.baseUrl
        .replace(/[/]ogc[/]wms[/]/, '/ogc/fis/');
      layerInfo = {
        ...shadowLayerInfo,
        image: (originalLayerInfo) ? originalLayerInfo.image : shadowLayerInfo.image,  // original layer's image is usually better
      };
    } else {
      url = selectedResult.baseUrl
        .replace('/ogc/wms/', '/ogc/fis/');
  ...
  handleRequestError(err) {
    ...
    if (err.response) {
      try {
        ...
        const errorMsg = (serverErrorMessages.length > 0) ? (`Error: ${serverErrorMessages[0].textContent}`) : (err.message);
        this.setState({
          fetching: false,
          errorMsg,
        });
  ...
  onDataReceived(fromDate, toDate, responseData) {
    // merge newly received data with existing:
    const channelIdWithCloudCoverage = (Object.keys(responseData).length > 1) ? (`C${Object.keys(responseData).length - 1}`) : (null);
    for (let channelId in responseData) {
      // exception: if there are exactly 2 channels then channel 'C1' is really cloud coverage info:
      if (channelIdWithCloudCoverage === channelId) {
  ...
  updateStateWithData() {
    ...
    for (let channelId in responseData) {
      const validStats = responseData[channelId].filter((stat) => (
        stat.basicStats.mean !== 'NaN' &&
        stat.histogram.bins.length === 10 &&
      ...
      const currentLineData = {
        ...
        coordinates: validStats.map((stat, index) => ({  // eslint-disable-line no-loop-func
          ...
          seriesIndex,
          ...stat.basicStats,
          median: stat.histogram.bins[5].lowEdge,
          p10: stat.histogram.bins[1].lowEdge,
          p90: stat.histogram.bins[9].lowEdge,
        }))
      }
      ...
      minY = Math.min(minY, validStats.reduce((prevValue, stat) => ( Math.min(prevValue, stat.basicStats.min)), Number.POSITIVE_INFINITY));
      maxY = Math.max(maxY, validStats.reduce((prevValue, stat) => ( Math.max(prevValue, stat.basicStats.max)), Number.NEGATIVE_INFINITY));
    ...
    return (
      <div className="interval-buttons">
        {this.TIME_INTERVALS.map((interval, index) => {
          return (
            (index === selectedTimeIntervalIndex) ? (
              <Button
              ...
              <Button
                key={`interval-button-${index}`}
                text={interval.label}
                onClick={ () => { this.onIntervalChange(index); } }
              />
            )
          )
        })}
    ...
    return (
      <div>
        ...
        <Button
          text="Retry"
          icon="refresh"
          onClick={ () => { this.ensureAllDataAvailable(); } }
        />
      </div>
    );
  }
  ...
  renderChart(drawLegend) {
    const { lineData, areaData, fromDate, toDate, minY, maxY, dataAvailableFromDate, fetching, maxCCAllowed } = this.state;
          ...
          <div>
            <svg width={600} height={50} className="legend">
              ...
              {lineData.map((serie, index) => {
                ...
                const DIST_VERT = 20;
                return (
                  <g key={`legendpart-${index}`} transform={`translate(${(DIST_HORIZ * index) % 600}, ${Math.floor((DIST_HORIZ * index) / 600) * DIST_VERT})`}>
                    <rect x={0} y={0} width={10} height={10} rx={2} ry={2} fill={this.chooseChartSeriesColor(index, lineData.length)} />
                    <text x={20} y={10}>{serie.title}</text>
                  </g>
                )
              })}
            </svg>
